...
ALTER TABLE DADOS_BASICOS ADD COLUMN ID_MIGRACAO TEXT NULL;
ALTER TABLE PACIENTE ADD COLUMN OBSERVACAO TEXT NULL;
...
LEFT JOIN PROFISSIONAL PP
	ON PP.ID = RP.ID_PROFISSIONAL_PAGAMENTO
WHERE RP.ID_PROFISSIONAL_PAGAMENTO IS NOT NULL
...
(
	...
	GROUP BY PROC.DESCRICAO, O.ID, PTP.ID
...
INSERT INTO FATURA_ITEM(DESCRICAO_ITEM, VALOR_ITEM, DATA_CRIACAO, TIPO_SALDO, ATIVO, FATURA_ID, ORCAMENTO_ITEM_ID)
SELECT
	OI.DESCRICAO,
	OI.VALOR,
...
WITH FATURA_PP AS (
	...
	INSERT INTO FATURA_ITEM(DESCRICAO_ITEM, VALOR_ITEM, DATA_CRIACAO, TIPO_SALDO, FATURA_ID, ATIVO, VALOR_COM_DESCONTO, FATURA_ITEM_ID)
	SELECT DISTINCT
		CASE
			WHEN P.TIPO_REMUNERACAO = 'POR' THEN 'Repasse de Profissional (' || LTRIM(TO_CHAR(
				CASE WHEN P.PERCENTUAL_REMUNERACAO IS NULL THEN 0::REAL
				ELSE P.PERCENTUAL_REMUNERACAO::REAL END, '999')) || '%) - ' || PROC.DESCRICAO
			WHEN P.TIPO_REMUNERACAO = 'PRO' THEN CASE
				WHEN PTP.ORTODONTICO = TRUE THEN 'Repasse de Profissional (Fixo: ' || REPLACE(LTRIM(TO_CHAR(
					CASE WHEN P.REMUNERACAO_MANUTENCAO IS NULL THEN 0::REAL
					ELSE P.REMUNERACAO_MANUTENCAO::REAL END, 'R$999D99')), '.', ',') || ') - ' || PROC.DESCRICAO
				ELSE 'Repasse de Profissional (Fixo: ' || REPLACE(LTRIM(TO_CHAR(
					CASE WHEN PROC.VALOR_REPASSE IS NULL THEN 0::REAL
					ELSE PROC.VALOR_REPASSE::REAL END, 'R$999D99')), '.', ',') || ') - ' || PROC.DESCRICAO END
		END,
		COALESCE(CASE
			WHEN P.TIPO_REMUNERACAO = 'POR' THEN ROUND(CAST((P.PERCENTUAL_REMUNERACAO / 100::FLOAT) * FI.VALOR_ITEM AS NUMERIC), 4)
			WHEN P.TIPO_REMUNERACAO = 'PRO' THEN CASE
				WHEN PTP.ORTODONTICO = TRUE THEN P.REMUNERACAO_MANUTENCAO
				ELSE PROC.VALOR_REPASSE END
		END, 0),
		CURRENT_TIMESTAMP,
		'E',
		FREP.ID,
	...
	INSERT INTO FATURA_ITEM(DESCRICAO_ITEM, VALOR_ITEM, DATA_CRIACAO, TIPO_SALDO, FATURA_ID, ATIVO, VALOR_COM_DESCONTO)
	SELECT
		CASE
			WHEN P.TIPO_REMUNERACAO = 'POR' THEN 'Repasse de Profissional (' || LTRIM(TO_CHAR(
				CASE WHEN P.PERCENTUAL_REMUNERACAO IS NULL THEN 0::REAL
		...
		END,
		COALESCE(CASE
			WHEN P.TIPO_REMUNERACAO = 'POR' THEN ROUND(CAST((P.PERCENTUAL_REMUNERACAO / 100::FLOAT) * PTP.VALOR_DESCONTO AS NUMERIC), 4)
			WHEN P.TIPO_REMUNERACAO = 'PRO' THEN CASE
				WHEN PTP.ORTODONTICO = TRUE THEN P.REMUNERACAO_MANUTENCAO
				ELSE PROC.VALOR_REPASSE END
		END, 0),
...
INSERT INTO TRANSFERENCIA_CONTA(ID_CONTA_ORIGEM, ID_CONTA_DESTINO, DESCRICAO, DATA, VALOR, ID_PROFISSIONAL, ATIVO)
SELECT
	CONTA_E.ID,
	CONTA_P.ID,
...
LEFT JOIN PROCEDIMENTO PROC ON PROC.ID = PTP.ID_PROCEDIMENTO
LEFT JOIN PROFISSIONAL PROF_RM ON PROF_RM.ID = RM."PP_ID"
LEFT JOIN DADOS_BASICOS PROF_RM_DB ON PROF_RM_DB.ID = PROF_RM.ID_DADOS_BASICOS
LEFT JOIN PROFISSIONAL PROF_FP ON PROF_FP.ID = PTP.FINALIZADO_POR
LEFT JOIN DADOS_BASICOS PROF_FP_DB ON PROF_FP_DB.ID = PROF_FP.ID_DADOS_BASICOS
WHERE PTP.FINALIZADO_POR IS NOT NULL;
