...
public class RepasseProfissionalComReciboMB extends LumeManagedBean<PlanoTratamentoProcedimento> {
    ...
    private Lancamento lancamentoCalculado;
...
//        }
    ...
    public void mostraCalculoRepasse(PlanoTratamentoProcedimento ptp) {
        ...
        if(validaPagamentoPaciente) {
          ...
          // List<Lancamento> lancamentos =  fatura.getLancamentos();  
           ...
           Lancamento lancamentoOrigem = faturaOrigem.getLancamentos().get(0);
           //TODO verificar como vamos mostrar quando tiver maid de um lançamento...
           //   for (Lancamento lancamento : lancamentos) {
           
               try {
                 
                   lancamentoCalculado = lancamentoOrigem;
                   ...
                   lancamentoCalculado.setDadosCalculoPercCustoDireto(lancamentoCalculado.getDadosTabelaValorTotalCustosDiretos().divide(lancamentoCalculado.getDadosTabelaValorTotalFatura(), 4, BigDecimal.ROUND_HALF_UP));
                   ...
                   lancamentoCalculado.setDadosCalculoTotalReducao(lancamentoCalculado.getDadosCalculoValorTaxa().add(lancamentoCalculado.getDadosCalculoValorTarifa()).add(lancamentoCalculado.getDadosCalculoValorTributo()));
                   ...
                   lancamentoCalculado.setDadosCalculoPercItemSemCusto(lancamentoCalculado.getDadosCalculoValorItemSemCusto().divide(lancamentoCalculado.getDadosTabelaValorTotalFatura(), 4, BigDecimal.ROUND_HALF_UP));
    ...
    public void verificaPendencias(PlanoTratamentoProcedimento ptp) {
        ...
        if(this.validaPagamentoPaciente) {
            if(ptp.getOrcamentoProcedimentos() == null || ptp.getOrcamentoProcedimentos().size() == 0 || ptp.getOrcamentoProcedimentos().get(0).getOrcamentoItem() == null ||
                    ptp.getOrcamentoProcedimentos().get(0).getOrcamentoItem().getOrcamento() == null 
                    || !ptp.getOrcamentoProcedimentos().get(0).getOrcamentoItem().getOrcamento().isAprovado()) {
...
//    }
    ...
    public Lancamento getLancamentoCalculado() {
        return lancamentoCalculado;
    }

    
    public void setLancamentoCalculado(Lancamento lancamentoCalculado) {
        this.lancamentoCalculado = lancamentoCalculado;
    }
