...
public class RelatorioGerencialBO extends BO<Paciente> {
    ...
    public List<Paciente> listPacientesInativos(Date inicio, Date fim, Empresa empresa) throws Exception {
        ...
        sb.append("SELECT ");
        sb.append(
                "(SELECT A.INICIO FROM AGENDAMENTO A WHERE A.ID_PACIENTE = P.ID AND A.STATUS NOT IN ('C','B') AND DATE(INICIO) <= CURRENT_DATE AND INICIO IS NOT NULL ORDER BY ID DESC FETCH FIRST 1 ROW ONLY) AS DATA_ULTIMA_ALTERACAO, ");
        sb.append(
                "(SELECT A.STATUS FROM AGENDAMENTO A WHERE A.ID_PACIENTE = P.ID AND A.STATUS NOT IN ('C','B') AND DATE(INICIO) <= CURRENT_DATE AND INICIO IS NOT NULL ORDER BY ID DESC FETCH FIRST 1 ROW ONLY) AS NOME_MAE ");
        sb.append(",P.ID,P.ID_DADOS_BASICOS ");
        sb.append("FROM PACIENTE P ");
        sb.append("WHERE P.ID_EMPRESA = ?1 ");
        sb.append("AND P.EXCLUIDO = 'N' ");
        sb.append("AND EXISTS (SELECT 1 FROM AGENDAMENTO A WHERE A.ID_PACIENTE = P.ID AND A.STATUS NOT IN ('C','B') AND DATE(A.INICIO) <= CURRENT_DATE ) ");
